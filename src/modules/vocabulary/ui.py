...
def render_word_card(word_data):
    """Render a word suggestion card with image, audio, and translation."""
    ...
    # --- DISPLAY IMAGE (outside the card) ---
    ...
    with st.container():
        ...
        with col1:
            st.markdown(f'<span class="vocab-word">{word}</span>', unsafe_allow_html=True)
        with col2:
            if level:
                st.markdown(f'<div style="text-align:right;"><span class="vocab-level">{level}</span></div>', unsafe_allow_html=True)
        
        # --- Translation ---
        if translation:
            st.markdown(f'<div class="vocab-translation">{translation}</div>', unsafe_allow_html=True)
        
        # --- Part of Speech ---
        if part_of_speech:
            st.markdown(f'<div class="vocab-pos">📖 {part_of_speech}</div>', unsafe_allow_html=True)
        
        # --- Definition ---
        st.markdown(f'<div class="vocab-definition">📖 {definition}</div>', unsafe_allow_html=True)
        ...
        # --- Audio ---
        if audio_url:
            st.markdown(f'<div style="color:#94a3b8;font-size:14px;margin-top:12px;">🔊 {phonetic if phonetic else "Listen"}</div>', unsafe_allow_html=True)
            st.audio(audio_url, format="audio/mpeg")
        elif phonetic:
            st.markdown(f'<div style="color:#94a3b8;font-size:14px;margin-top:12px;">🔊 {phonetic} <span style="color:#64748b;font-size:12px;">(Audio not available)</span></div>', unsafe_allow_html=True)
